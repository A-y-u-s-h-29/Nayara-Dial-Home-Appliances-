...
const FloatingButtons = () => {
  ...
  return (
    <div
      className={`fixed bottom-8 right-8 z-50 flex flex-col gap-4 transition-all duration-500 ${
    ...
    >
      ...
      {/* Phone Button - Increased Size */}
      <button
        onClick={handleCall}
        className="group relative bg-gradient-to-br from-green-500 to-green-600 text-white p-4 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 hover:shadow-3xl hover:-rotate-3"
        aria-label="Call"
      >
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Ripple effect */}
        <span className="absolute inset-0 rounded-full animate-ping bg-green-500/40 opacity-0 group-hover:opacity-100 transition-opacity"></span>
      </button>
    </div>
  );
};
